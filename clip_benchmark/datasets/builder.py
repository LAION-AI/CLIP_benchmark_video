...
def build_dataset(dataset_name, root="root", transform=None, split="test", download=True, annotation_file=None, language="en", task="zeroshot_classification", wds_cache_dir=None, custom_classname_file=None, custom_template_file=None, force_use_transform=False, include_sample_id=False, **kwargs):
    """
    ...
    include_sample_id: bool
        Include WebDataset ``__key__`` values in returned retrieval samples.
    ...
    if dataset_name == "cifar10":
        ...
        ds = build_wds_dataset(name, transform=transform, split=split, data_dir=root, cache_dir=wds_cache_dir, force_use_transform=force_use_transform, include_sample_id=include_sample_id)
...
def build_wds_dataset(dataset_name, transform, split="test", data_dir="root", cache_dir=None, force_use_transform=False, include_sample_id=False):
    ...
    if dataset_type == "retrieval":
        tuple_fields = (img_extensions, "txt", "__key__") if include_sample_id else (img_extensions, "txt")
        tuple_transforms = (transform, str.splitlines, None) if include_sample_id else (transform, str.splitlines)
        dataset = (dataset
            .to_tuple(*tuple_fields)
            .map_tuple(*tuple_transforms)
        )
        dataset.classes = dataset.templates = None
    elif dataset_type == "video_retrieval":
        tuple_fields = (video_extensions, "txt", "__key__") if include_sample_id else (video_extensions, "txt")
        tuple_transforms = (transform, str.splitlines, None) if include_sample_id else (transform, str.splitlines)
        dataset = (dataset
            .to_tuple(*tuple_fields)
            .map_tuple(*tuple_transforms)
